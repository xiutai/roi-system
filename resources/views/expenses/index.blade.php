    ...
    <div class="card-body">
        <form method="GET" action="{{ route('expenses.index') }}" class="row g-3">
            ...
            <div class="col-md-3 d-flex align-items-end">
                ...
                <a href="{{ route('expenses.index') }}" class="btn btn-secondary ms-2">重置</a>
            </div>
        </form>
    </div>
</div>
...
<div class="alert alert-info">
    <strong>默认消耗：</strong>
    <ul class="mb-0">
        @forelse($defaultExpenses as $expense)
            <li>{{ $expense->channel->name }}: {{ $expense->amount }}</li>
        @empty
            <li>尚未设置默认消耗</li>
        @endforelse
    </ul>
                            ...
                            @foreach($expenses as $expense)
                                <tr>
                                    <td>
                                        ...
                                        <div class="form-check">
                                            <input class="form-check-input select-item" type="checkbox" name="ids[]" value="{{ $expense->id }}">
                                        </div>
                                        @endif
                                    </td>
                                    <td>{{ $expense->date->format('Y-m-d') }}</td>
                                    <td>{{ $expense->channel->name }}</td>
                                    <td>{{ $expense->amount }}</td>
                                    <td>{{ $expense->created_at->format('Y-m-d H:i') }}</td>
                                    <td>
                                        <a href="{{ route('expenses.edit', $expense) }}" class="btn btn-sm btn-primary">编辑</a>
                                        @if(!$expense->is_default)
                                        <a href="{{ route('test.expense.delete', $expense->id) }}" class="btn btn-sm btn-danger" onclick="return confirm('确定要删除这条消耗记录吗？')">删除</a>
                                        @endif
                                    </td>
                                </tr>
                            @endforeach
...
<div class="modal fade" id="batchModal" tabindex="-1" aria-labelledby="batchModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <form action="{{ route('expenses.batch') }}" method="POST">
                @csrf
                <div class="modal-header">
                    <h5 class="modal-title" id="batchModalLabel">批量设置消耗</h5>
...
<div class="modal fade" id="defaultModal" tabindex="-1" aria-labelledby="defaultModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <form action="{{ route('expenses.update_default') }}" method="POST">
                @csrf
                <div class="modal-header">
                    <h5 class="modal-title" id="defaultModalLabel">设置默认消耗</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <div class="mb-3">
                    ...
                    <div class="mb-3">
                        <label for="default_amount" class="form-label">默认消耗金额</label>
                        <input type="number" step="0.01" min="0" class="form-control" id="default_amount" name="default_amount" required>
                        <small class="form-text text-muted">当某日期没有设置特定消耗时，将使用此默认值。</small>
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">取消</button>
